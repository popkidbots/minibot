...
const BOT_NAME = process.env.BOT_NAME || "The POPKID";
const OWNER_NAME = process.env.OWNER_NAME || "POPKID";
...
const AUTO_STATUS_MSG = process.env.AUTO_STATUS_MSG || "YOUR STATUS HAS BEEN SEEN BY POPKID🤭❤️";
const DEV = process.env.DEV || 'POPKID';
...
// Modern POPKID menu with sections and clean layout
const now = new Date();
...
⚡ *Cmds*  : ${allCommands.length} loaded

━━━━━━━━━━━━━━━━━━━━━━━\n`;

for (const [tag, cmds] of Object.entries(commandsByTag)) {
    const icon = categoryIcons[tag.toLowerCase()] || '📂';
...
function gracefulShutdown() {
  ...
  isShuttingDown = true;
  console.log("\n🛑 Shutting down The popkid MD server...");
